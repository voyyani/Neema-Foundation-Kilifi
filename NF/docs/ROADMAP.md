...
| 2 | **Landing** | `src/pages/Landing.tsx` + `Hero`, `Mission`, `Problem`, `Programs`, `Impact`, `Stories`, `Events`, `Action`, `Contact` | Persuade | `Hero.tsx` is rebuilt from the approved comp, not refactored — the 1,389-line file goes away. Keep the section keys the maintenance registry expects (`hero`, `mission`, …). |
