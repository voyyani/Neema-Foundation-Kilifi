  ...
  return (
    <div
      ...
      className={`bg-white border rounded-xl overflow-hidden shadow-sm flex flex-col transition-shadow ${
        isDragging ? 'shadow-2xl ring-2 ring-brand-600/60' : 'hover:shadow-md'
      } ${image.is_cover ? 'ring-2 ring-brand-600' : ''}`}
    >
      {/* Thumbnail */}
      <div className="relative group aspect-video bg-gray-100">
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-200 flex items-start justify-between p-2 opacity-0 group-hover:opacity-100">
          ...
          <button
            ...
            className="p-1.5 bg-danger-500/90 rounded-lg text-white hover:bg-danger-600 transition-colors"
            title="Delete photo"
          >
        ...
        {/* Cover badge */}
        {image.is_cover && (
          <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-brand-600 text-white text-xs font-semibold px-2 py-0.5 rounded-full shadow">
      ...
      <div className="p-3 space-y-2 flex-1 flex flex-col">
        <div>
          ...
          <input
            ...
            className="w-full text-xs px-2 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-600 focus:border-brand-600 text-gray-700 placeholder-gray-300"
          />
        </div>
        <div>
          <label className="block text-[11px] font-medium text-gray-500 mb-0.5 uppercase tracking-wide">
        ...
        <div className="pt-1 mt-auto">
          <button
            ...
            className={`w-full flex items-center justify-center gap-1.5 text-xs font-medium py-1.5 rounded-lg border transition-colors ${
              image.is_cover
                ? 'bg-brand-600/10 text-brand-600 border-brand-600/30 cursor-default'
                : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50 hover:text-brand-600 hover:border-brand-600/40'
            }`}
          >
            {setCover.isPending ? (
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white border rounded-xl overflow-hidden shadow-sm ${
        entry.status === 'error' ? 'border-danger-200' : 'border-gray-200'
      }`}
    >
      {/* Thumbnail */}
      <div className="aspect-video bg-gray-100 relative overflow-hidden">
        ...
        {entry.status === 'error' && (
          <div className="absolute inset-0 flex items-center justify-center bg-danger-500/20">
            <AlertCircle className="w-7 h-7 text-danger-600 drop-shadow" />
          </div>
        )}
      ...
      <div className="px-2 py-1.5">
        ...
        {entry.status === 'error' && (
          <p className="text-[11px] text-danger-500">{entry.errorMsg ?? 'Upload failed'}</p>
        )}
      </div>
    </motion.div>
  );
      ...
      >
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 px-4 py-2 bg-danger-600 text-white rounded-lg hover:bg-danger-700 text-sm font-medium flex items-center justify-center gap-2"
          >
    ...
    <div className="space-y-6">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
          isDragActive
            ? 'border-brand-600 bg-danger-50 scale-[1.01]'
            : 'border-gray-300 bg-gray-50 hover:border-brand-600/40 hover:bg-danger-50/30'
        }`}
      >
        <input {...getInputProps()} />
        <Upload
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-brand-600 text-white text-sm font-medium rounded-lg hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
              >
